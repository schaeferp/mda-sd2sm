...
namespace Egp.Mda.Transformation.Core
{
    public class ScenarioService : IScenarioService
    {
        ...
        public IEnumerable<Scenario> From(XmiSequenceDiagramModel xmiModel)
        {
            ...
            foreach (var sequenceDiagram in collaborations)
            {
                ...
                foreach (var fragment in sequenceDiagram.Fragments.Values)
                {
                    switch (fragment.XmiType)
                    {
                        ...
                        case UmlMessageOccurenceSpecificationAttributeValue:
                        {
                            ...
                            if (participantIsSender) continue;
                            ...
                            foreach (var lifelineId in fragment.Covered)
                            {
                                ...
                                var lastInvocationExists = lastInvocationPerParticipant.TryGetValue(participant,
                                    out lastInvocation);
                ...
                foreach (var lastInvocation in lastInvocationPerParticipant.Values)
                {
                    if (null == lastInvocation.PostStateInvariant)
